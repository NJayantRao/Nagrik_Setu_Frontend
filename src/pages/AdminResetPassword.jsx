...
function AdminResetPassword(){
    ...
    const {setAdminUniqueId,adminUniqueId,adminPassword,setAdminPassword,}= useContext(AdminDataContext)
  ...
  toast[type](message, {
    ...
    async function submitHandler(e){
        e.preventDefault()
        try {
            const response= await axios.put(`${import.meta.env.VITE_LOCAL_URL}/admin/resetPassword`,{uniqueId:adminUniqueId,otp,newPassword:adminPassword})
            ...
            <form className="flex flex-col gap-5 w-full p-3" onSubmit={(e)=>{
                ...
                <div className="font-semibold text-gray-600 text-lg ">
                    ...
                    <div className="relative">
                        <input type={`${showPassword?"text":"password"}`} placeholder="Enter New Password" name="password" className={` py-2 px-4 rounded-xl w-full text-gray-600 text-sm shadow-sm focus:outline-none focus:ring-2 focus:bg-[#e0e7ff] focus:ring-blue-400 ${adminPassword?"bg-[#e8f0ff]":"bg-gray-200"}`} 
                    value={adminPassword} 
                    onChange={(e)=>{
                ...
                <div className="w-full flex justify-center mt-0.5">
                    <button type="submit" className="bg-blue-600 p-2 w-1/2 rounded-full text-xl text-white font-bold hover:scale-105 hover:cursor-pointer hover:bg-blue-700 hover:ease-in-out">Reset Password</button>
                </div>
            </form>
           </div>
        </div>
