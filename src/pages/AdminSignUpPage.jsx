...
function AdminSignUpPage(){
    const{adminName,setAdminName,adminEmail,setAdminEmail,adminPassword,setAdminPassword,adminAddress,setAdminAddress,adminPhone,setAdminPhone}= useContext(AdminDataContext)
    ...
    async function submitHandler(e){
        ...
        try {
            const response= await axios.post(`${import.meta.env.VITE_LOCAL_URL}/admin/signup`,{
                name:adminName,email:adminEmail,password:adminPassword,phone:adminPhone,address:adminAddress
            ...
            <form className="flex flex-col gap-2 w-full" onSubmit={(e)=>{
                ...
                <div className="font-semibold text-gray-600 text-lg ">
                    ...
                    <div className="relative">
                        <input type={`${showPassword?"text":"password"}`} placeholder="Enter Your Password" name="password" className={` py-2 px-4 rounded-xl w-full text-gray-600 text-sm shadow-sm focus:outline-none focus:ring-2 focus:bg-[#e0e7ff] focus:ring-blue-400 ${adminPassword?"bg-[#e8f0ff]":"bg-gray-200"}`} 
                    value={adminPassword} 
                    onChange={(e)=>{
                ...
                <div className="font-semibold text-gray-600 text-lg">
                    Phone
                    <input type="text" placeholder="Enter Your Contact number" name="phone" className={` py-2 px-4 rounded-xl w-full text-gray-600 text-sm shadow-sm focus:outline-none focus:ring-2 focus:bg-[#e0e7ff] focus:ring-blue-400 ${adminPhone?"bg-[#e8f0ff]":"bg-gray-200"}`} 
                    value={adminPhone} 
                    onChange={(e)=>{
                ...
                <div className="font-semibold text-gray-600 text-lg">
                    Address
                    <input type="text" placeholder="Enter Your City" name="address" className={` py-2 px-4 rounded-xl w-full text-gray-600 text-sm shadow-sm focus:outline-none focus:ring-2 focus:bg-[#e0e7ff] focus:ring-blue-400 ${adminAddress?"bg-[#e8f0ff]":"bg-gray-200"}`} 
                    value={adminAddress} 
                    onChange={(e)=>{
